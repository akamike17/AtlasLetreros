...
test('texto editable, persistencia, undo/redo y preview separado del envío', async ({ page }) => {
  ...
  await page.locator('#itText').fill('ATLAS');
...
test('fuentes bitmap, edición de texto, imagen y biblioteca multiproyecto', async ({ page }) => {
  ...
  const addText = async (text, font) => {
    ...
    await page.locator('#itText').fill(text);
    await page.locator('#itFont').selectOption(font);
    await page.locator('#itText').press('Control+Enter');
  };
  ...
  if (await page.locator('#textInspector').isVisible()) {
    await page.locator('#itText').fill('EDITADO');
    await page.locator('#itText').press('Control+Enter');
  }
  ...
  await page.getByRole('button', { name: 'Enviar', exact: true }).click();
  await expect(page.locator('#runtimeState')).toHaveText('PLAYING');
  expect(semanticPayload.scene.layers).toHaveLength(1);
  expect(semanticPayload.scene.layers[0].elements).toHaveLength(3);
