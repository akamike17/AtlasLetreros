...
export class PhysicalTransport {
 ...
 async receive(bytes,expected,signal){const packet=JSON.parse(new TextDecoder().decode(bytes));const response=await fetch('/api/devices/upload',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({checksum:expected,packet}),signal});const result=await response.json();if(!response.ok||result.checksum!==expected||typeof result.candidateId!=='string'||!/^[0-9a-f]{32}$/i.test(result.candidateId)||result.verified!==true||result.activated===true)throw new Error(result.message||'El firmware no confirmó la verificación.');this.lastChecksum={checksum:result.checksum,candidateId:result.candidateId};return result.checksum;}
